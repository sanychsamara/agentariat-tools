...
try {
  ...
  if ($LASTEXITCODE -ne 0) { [Console]::Error.WriteLine("install: the client could not be fetched and verified; the active bundle in $Dir is untouched"); exit 2 }
  python "$stage\get-client.py" --check
  if ($LASTEXITCODE -ne 0) { [Console]::Error.WriteLine("install: the staged client does not match the pin; the active bundle is untouched"); exit 2 }
  ...
  if ($LASTEXITCODE -ne 0) { [Console]::Error.WriteLine("install: the staged watcher does not import with its client; the active bundle is untouched"); exit 2 }
  if ((Test-Path $Dir) -and -not $Force) {
    ...
    if ($live.Count -gt 0) { [Console]::Error.WriteLine("install: a watcher or notifier runs from $Dir (pids: $($live.Name -join ' ')); stop it first, or pass -Force and restart it afterwards"); exit 1 }
  }
  ...
  try { Move-Item $stage $Dir } catch {
    # Diagnostics here must not throw ($ErrorActionPreference is Stop), or the restoration below never runs.
    [Console]::Error.WriteLine("install: activation failed ($_)")
    if (Test-Path $previous) {
      try { Move-Item $previous $Dir; [Console]::Error.WriteLine("install: the previous bundle is back at $Dir; nothing changed") }
      catch { $keepStage = $true; [Console]::Error.WriteLine("install: RESTORATION FAILED ($_): the previous bundle is at $previous, the candidate at $stage; move one to $Dir by hand"); exit 2 }
    }
    exit 2
  }
